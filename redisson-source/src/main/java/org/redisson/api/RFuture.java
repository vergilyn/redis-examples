...
/**
 * Represents(代表,表示) the result of an asynchronous computation(计算);
 * 表示一个异步计算的结果.
...
public interface RFuture<V> extends java.util.concurrent.Future<V> {
     ...
     * <br/> 返回没有阻塞的结果. 如果future还没有完成, 这将返回{null}.
     *
     * <br/> As it is possible that a {@code null} value is used to mark the future as successful you also need to check
    ...
    /**
     * Waits for this future to be completed within the specified time limit. <br/>
     * 等待这个<code>RFuture</code>在规定的时间<code>timeoutMillis</code>内完成. <br/>
     *
     * @param timeoutMillis - timeout value
     * @return {@code true} if and only if the future was completed within
     *         the specified time limit. (true: 仅当这个<code>RFuture</code>在<code>timeoutMillis<code/>内完成)
     *
     * @throws InterruptedException
     *         if the current thread was interrupted
     */
     ...
     * specified listener is notified(通知) when this future is
     * {@linkplain #isDone() done}.  If this future is already
     * completed, the specified listener is notified immediately(立即);
     *
     * <br/>添加一个指定的future-listener, 当这个future执行完成会立即通知listener.
